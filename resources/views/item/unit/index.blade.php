                ...
                <div class="card-body">
                    <div class="container-fluid">
                        <div class="table-responsive">
                            <table class="table table-bordered hover compact dtTable" id="dtManageItemUom">
                                <thead>
                                    <tr role="row">
                                    ...
                                    <tr role="row" class="filter">
                                        <td class="bg-primary text-white">
                                            {{ Form::text('name', null, array('class' => 'w-100 form-control text-filter', 'data-column' => '0')) }}
                                        </td>

                                        <td class="bg-primary text-white">
                                            {{ Form::text('description', null, array('class' => 'w-100 form-control text-filter', 'data-column' => '1')) }}
                                        </td>
...
@push('scripts')
    <script type="text/javascript">
        $(document).ready(function() {
            initializeManageItemUnitDatatable();
        });

        function initializeManageItemUnitDatatable() {
            let dtTable = $('#dtManageItemUom').DataTable({
                processing: true,
                serverSide: true,
                orderCellsTop: true,
                ajax: '{!! route('manage.item.unit.list') !!}',
                columns: [
                    { data: 'name', name: 'name' },
